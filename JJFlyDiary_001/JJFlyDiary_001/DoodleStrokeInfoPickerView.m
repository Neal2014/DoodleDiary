...
@implementation DoodleStrokeInfoPickerView
...
{
    //取得选择的是第0列的哪一行
    NSInteger rowOfColorComponent = [pickerView selectedRowInComponent:0];
    //取得选择的是第1列的哪一行
    NSInteger rowOfSizeComponent = [pickerView selectedRowInComponent:1];
    //取得所选列所选行的视图
    UIView *ViewOfColorComponent = [pickerView viewForRow:rowOfColorComponent forComponent:0];
    UIView *ViewOfSizeComponent = [pickerView viewForRow:rowOfSizeComponent forComponent:1];
    //取得取得所选行所选列上的视图的子视图
    ...
    //创建model
    DoodleModel *doodleModel = [DoodleModel shareDoodleDiaryModel];
    
    doodleModel.doodleStrokeWidth=[viewOnViewOfSizeComponent.text floatValue];
    doodleModel.doodleStrokeColor=viewOnViewOfColorComponent.backgroundColor;
    
    //代理传值
    if (nil != _doodleStrokeDelegate && [_doodleStrokeDelegate respondsToSelector:@selector(changeDoodleStrokeInfoForDoodleView:)]) {
        [_doodleStrokeDelegate changeDoodleStrokeInfoForDoodleView:doodleModel];
    }
}
